    ...
    <div class="container">
        <form action="{{ route('simpansoal') }}" method="post">
                ...
                <div class="col-8 col-md-3">
                    <select class="form-select" id="inputGroupSelect01" name="training">
                        @foreach ($trainings as $training)
                            <option value="{{ $training->id }}">{{ $training->id }}. {{ $training->nama_training }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="row justify-content-evenly gap-md-5 my-3 gap-2">
                ...
                <button id="simpanSoalBtn" type="submit" class="btn btn-success col">Simpan Soal</button>
            </div>
        </form>

        <form action="/panitia/test" class="row justify-content-end my-3">
            <button type="submit" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal">Kembali</button>
        </form>
    </div>
